...
required_cv12_columns <- c(
  "Trait", "CV", "MeanCorrelation", "SDCorrelation",
  "FullEffects", "Group", "Model"
)

missing_cv12_columns <- setdiff(required_cv12_columns, names(cv12_summary))

if (length(missing_cv12_columns) > 0) {
  stop(
    "cv12_summary does not contain the columns required by the figure-cv12 chunk: ",
    paste(missing_cv12_columns, collapse = ", ")
  )
}
...
if ("Pathway" %in% names(cv12_plot_data)) {
  cv12_plot_data <- cv12_plot_data %>%
    dplyr::mutate(
      Group = dplyr::case_when(
        Pathway == "Genomic pathway" ~ "Genomic",
        Pathway == "Phenomic pathway" ~ "Phenomic",
...
cv12_plot_data <- cv12_plot_data %>%
  dplyr::mutate(
    ...
    Model = stringr::str_replace_all(as.character(Model), "Eta", "M"),
    ModelNumber = as.integer(stringr::str_extract(Model, "\\d+"))
  )

build_cv12_panel <- function(current_trait, current_cv, panel_title, show_y_axis_title = TRUE) {
  
  trait_cv_data <- cv12_plot_data %>%
    dplyr::filter(Trait == current_trait, CV == current_cv) %>%
    dplyr::arrange(Group, ModelNumber) %>%
    dplyr::mutate(
      Model = factor(Model, levels = unique(Model))
    )
  
  if (nrow(trait_cv_data) == 0) {
    stop(
      "No data available for Trait = ", current_trait,
      " and CV = ", current_cv, "."
    )
  }
  ...
  ggplot(
    trait_cv_data,
    aes(x = Model, y = MeanCorrelation, fill = Group)
  ) +
    geom_col(
      width = 0.75,
      color = "grey30",
    ...
    scale_y_continuous(
      name = if (show_y_axis_title) "Predictive correlation" else NULL,
...
cv12_combined_plot_objects <- list()

for (current_trait in unique(cv12_plot_data$Trait)) {
  
  plot_cv1 <- build_cv12_panel(
  ...
  trait_title <- dplyr::case_when(
    current_trait == "GY" ~ "Predictive ability for grain yield (GY)",
    current_trait == "KW" ~ "Predictive ability for kernel weight (KW)",
    TRUE ~ paste("Predictive ability for", current_trait)
  )
  
  cv12_combined_plot_objects[[current_trait]] <-
    (plot_cv1 + plot_cv2) +
  ...
  print(cv12_combined_plot_objects[[current_trait]])
}
```

```{r save-figure-cv12, include=FALSE}
if (!exists("cv12_combined_plot_objects")) {
  stop("The object cv12_combined_plot_objects was not created before the save-figure-cv12 chunk.")
}

for (current_trait in names(cv12_combined_plot_objects)) {
  
  ggsave(
    filename = file.path(
      figures_dir,
      paste0("prediction_", current_trait, "_CV1_CV2_combined.tiff")
    ),
    plot = cv12_combined_plot_objects[[current_trait]],
    width = 16,
    height = 10,
    dpi = 600,
    compression = "lzw",
...
for (current_trait in cvloo_traits) {
  for (current_cv in cvloo_schemes) {
    ...
    trait_cv_data <- trait_cv_data %>%
      filter(Trait == current_trait, CV == current_cv) %>%
      arrange(PlotOrder) %>%
      mutate(FullEffectsWrapped = factor(FullEffectsWrapped, levels = model_order_wrapped_en)) %>%
      mutate(Group = factor(Group, levels = c("Genomic", "Phenomic", "Combined"))) %>%
      arrange(Group, as.numeric(stringr::str_extract(Model, "\\d+"))) %>%
      mutate(Model = str_replace_all(Model, "Eta", "M"),
        Model = factor(Model, levels = unique(Model)))
    ...
    cvloo_plot_objects[[plot_key]] <- ggplot(
      ...
      coord_cartesian(ylim = c(plot_lower_limit_trait_cv, 1)) +
