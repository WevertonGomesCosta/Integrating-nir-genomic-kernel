...
if (length(missing_cv12_summary_cols) > 0) {
  stop(
    "cv12_summary ficou sem colunas obrigatórias após a carga: ",
...
if (length(missing_cvloo_summary_cols) > 0) {
  stop(
    "cvloo_summary ficou sem colunas obrigatórias após a carga: ",
...
if (length(missing_cv12_w_effect_cols) > 0) {
  stop(
    "prediction_cv1_cv2_w_effect_pairs.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cvloo_w_effect_cols) > 0) {
  stop(
    "prediction_cv0_cv00_w_effect_pairs.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cv12_kernel_cols) > 0) {
  stop(
    "prediction_cv1_cv2_kernel_family_compare.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cvloo_kernel_cols) > 0) {
  stop(
    "prediction_cv0_cv00_kernel_family_compare.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cv12_display_cols) > 0) {
  stop(
    "prediction_cv1_cv2_display_table.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cvloo_display_cols) > 0) {
  stop(
    "prediction_cv0_cv00_display_table.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cv12_top3_cols) > 0) {
  stop(
    "prediction_cv1_cv2_top3.csv ficou sem colunas obrigatórias: ",
...
if (length(missing_cvloo_top3_cols) > 0) {
  stop(
    "prediction_cv0_cv00_top3.csv ficou sem colunas obrigatórias: ",
...
for (current_trait in cv12_traits) {
  for (current_cv in cv12_schemes) {
    trait_cv_data <- cv12_plot_data %>%
      ...
      mutate(
        FullEffectsWrapped = factor(
          FullEffectsWrapped,
          levels = unique(FullEffectsWrapped)
        )
      )
    ...
    ggsave(
      file.path(figures_dir, paste0("prediction_", current_trait, "_", tolower(current_cv), ".tiff")),
      plot = p_trait_cv12,
      width = 18,
      height = 11,
      dpi = 300,
...
for (current_trait in cvloo_traits) {
  for (current_cv in cvloo_schemes) {
    ...
    model_order <- trait_cv_data %>%
      group_by(FullEffectsWrapped) %>%
      summarise(OrderMean = mean(MeanCorrelation, na.rm = TRUE), .groups = "drop") %>%
      arrange(OrderMean) %>%
      pull(FullEffectsWrapped)
