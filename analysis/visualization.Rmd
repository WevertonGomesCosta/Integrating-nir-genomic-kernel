...
if (length(missing_tables) > 0) {
  stop(
    "Este arquivo de visualização depende de tabelas já geradas em output/tables/.\n",
    "Arquivos ausentes: ",
...
```{r table-cv12-top3}
if (!exists("cv12_top3")) {
  stop("O objeto cv12_top3 não foi carregado antes do chunk table-cv12-top3.")
}
if (!all(c("Trait", "CV", "ShortDescription", "MeanCorrelation", "SDCorrelation") %in% names(cv12_top3))) {
  stop("cv12_top3 não contém as colunas exigidas pelo chunk table-cv12-top3.")
}
...
if (!exists("cvloo_top3")) {
  stop("O objeto cvloo_top3 não foi carregado antes do chunk table-cvloo-top3.")
}
if (!all(c("Trait", "CV", "Env_leave", "ShortDescription", "MeanCorrelation", "SDCorrelation") %in% names(cvloo_top3))) {
  stop("cvloo_top3 não contém as colunas exigidas pelo chunk table-cvloo-top3.")
}
...
if (!exists("cv12_w_effect")) {
  stop("O objeto cv12_w_effect não foi carregado antes do chunk table-cv12-w-effect.")
}
if (!all(c("Comparison", "Trait", "CV", "MeanWithoutW", "MeanWithW", "DeltaWithW") %in% names(cv12_w_effect))) {
  stop("cv12_w_effect não contém as colunas exigidas pelo chunk table-cv12-w-effect.")
}
...
if (!exists("cvloo_w_effect")) {
  stop("O objeto cvloo_w_effect não foi carregado antes do chunk table-cvloo-w-effect.")
}
if (!all(c("Comparison", "Trait", "CV", "Env_leave", "MeanWithoutW", "MeanWithW", "DeltaWithW") %in% names(cvloo_w_effect))) {
  stop("cvloo_w_effect não contém as colunas exigidas pelo chunk table-cvloo-w-effect.")
}
...
if (!exists("cv12_kernel_family_compare")) {
  stop("O objeto cv12_kernel_family_compare não foi carregado antes do chunk table-cv12-kernel-family.")
}
if (!all(c("Trait", "CV", "Pathway", "WeatherStatus", "Linear", "Gaussian", "Arc-cosine") %in% names(cv12_kernel_family_compare))) {
  stop("cv12_kernel_family_compare não contém as colunas exigidas pelo chunk table-cv12-kernel-family.")
}
...
if (!exists("cvloo_kernel_family_compare")) {
  stop("O objeto cvloo_kernel_family_compare não foi carregado antes do chunk table-cvloo-kernel-family.")
}
if (!all(c("Trait", "CV", "Env_leave", "Pathway", "WeatherStatus", "Linear", "Gaussian", "Arc-cosine") %in% names(cvloo_kernel_family_compare))) {
  stop("cvloo_kernel_family_compare não contém as colunas exigidas pelo chunk table-cvloo-kernel-family.")
}
  ...
  for (current_cv in cv12_schemes) {
    ...
    p_trait_cv12 <- ggplot(
      ...
      scale_y_continuous(
        ...
        limits = c(lower_limit_trait_cv - 0.03, upper_limit_trait_cv + 0.03),
