...
The document starts by defining global knitting options so that figures, messages, and code blocks appear consistently throughout the tutorial.

This chunk configures the document and keeps graphical and reporting options consistent across the
module. It prepares the session so that the remaining steps focus only on the climate-processing
workflow.
...
**Next step**
- `matrizes.Rmd` uses the climate-derived outputs generated here to align environmental information with the observational structure and build the weather kernel (`W`).
...
The code below loads the packages required for data download, data wrangling, plotting, and panel composition before any climate processing begins.

This chunk loads the packages required for climate acquisition, reshaping, summary, and plotting.
Grouping the dependencies here makes the downstream steps easier to follow.
...
This chunk defines the time window, location, and general controls used to query NASA POWER and to
organize the climatic records. These parameters determine the scope of the environmental data
...
This chunk downloads the first block of climate records from NASA POWER. It establishes the raw
daily data that will later be cleaned and harmonized across seasons.
...
This chunk completes the climate download for the remaining period and combines the results into a
unified daily table. The goal is to obtain a continuous climatic record covering the target growing
seasons.
...
This chunk defines reusable graphical settings for the figures presented below. Centralizing these
settings helps keep the visual summaries comparable across variables.
...
This chunk builds the first precipitation plot, emphasizing the day-to-day profile across seasons.
It provides the broadest visual overview of rainfall dynamics in the study period.
...
This chunk reorganizes precipitation by month and day to make within-season contrasts easier to
compare between years. The resulting figure highlights seasonal timing more clearly than a single
continuous series.
...
This chunk aggregates daily precipitation into monthly totals. The summary is useful for
highlighting broader seasonal accumulation patterns that may not be obvious from daily values alone.
...
This chunk summarizes the daily minimum, mean, and maximum temperatures observed during the crop
seasons. It helps describe thermal contrasts that may later influence prediction and interaction
patterns.
...
This chunk combines the main precipitation and temperature panels into a single figure. The joint
view gives a compact summary of the major climatic differences across seasons.
...
This chunk generates complementary daily plots for solar radiation, wind speed, and relative
humidity. These variables expand the environmental characterization beyond precipitation and
temperature.
...
This chunk combines the additional climatic variables into a multi-panel summary. Presenting these
series together helps compare their seasonal behavior at a glance.
...
The next block computes the annual and expanded climate covariates that will later be used to build the weather-derived inputs of the prediction pipeline.

This chunk derives the climate covariates that will later be used in the kernel-building and
prediction modules. It converts the raw daily series into structured variables aligned with the
analytical pipeline.
...
This chunk saves the processed climatic outputs for downstream use. Writing these files closes the
climate module and provides stable inputs for the remaining stages of the workflow.
