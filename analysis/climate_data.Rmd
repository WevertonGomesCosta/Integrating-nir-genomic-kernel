...
This chunk sets the global knitting options used across the document so that figures, messages, and code blocks are displayed consistently in the rendered tutorial.
...
This chunk loads the packages needed for data download, wrangling, plotting, and panel composition before any climate processing begins.
...
This chunk defines the locations, years, and basic metadata that control which weather series will be downloaded and how the downstream summaries will be labeled.
...
This chunk downloads the main daily weather data from NASA POWER and stores the raw table that later steps use for inspection, plotting, and feature engineering.
...
This chunk expands the download logic to collect the complementary weather variables required to complete the final climate dataset used by the pipeline.
...
This chunk defines shared graphical settings so that the precipitation and temperature figures follow the same visual standard throughout the tutorial.
...
This chunk creates the first precipitation overview figure, giving a broad visual summary of rainfall behavior across the study conditions.
...
This chunk reorganizes the precipitation data into grouped views that make between-environment patterns easier to compare.
...
This chunk summarizes precipitation at the monthly level so that seasonal contrasts can be inspected more clearly than in the daily series alone.
...
This chunk produces the temperature figure used to compare the thermal profile of the evaluated environments over time.
...
This chunk combines the main precipitation and temperature panels into a single layout that can be used directly in reports or teaching materials.
...
This chunk builds the complementary plots that document secondary weather patterns not shown in the main combined panel.
...
This chunk arranges the complementary weather plots into a compact multi-panel figure for downstream inspection.
...
This chunk computes the annual and expanded climate covariates that will later be used to build the weather-derived inputs of the prediction pipeline.
...
This chunk writes the raw and engineered climate outputs to disk so that later modules can reuse them without repeating the download and preprocessing stages.
