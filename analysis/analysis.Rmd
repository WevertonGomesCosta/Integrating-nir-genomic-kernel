...
*Subgrupo 2.2.3: Ambas Interações (G×E + G×W)*
*(Hipótese: "Tanto a interação categórica (`GE`) quanto a baseada no clima (`GW`) capturam variações distintas e úteis.")*

| Modelo | Estrutura de Efeitos |
| :--- | :--- |
| **E+W+G+GE+GW** | E+W + Genômico (Linear) + Ambas Interações |
| **E+W+P+PE+PW** | E+W + Fenômico (Linear) + Ambas Interações |
| **E+W+G+P+GE+PE+GW+PW** | E+W + G + P + Todas Interações |
| **E+W+GGK+GGKE+GGKW** | E+W + G (Gaussiano) + Ambas Interações |
| **... (etc.)** | *(Modelos equivalentes para PGK, GAK, PAK)* |
| **E+W+GAK+PAK+GAKE+PAKE+GAKW+PAKW** | E+W + G/P (Arc-Kernel) + Todas Interações |
...
# Grupo 1: Modelos Base (Efeito Categórico E)
    ...
    # Grupo 4: Modelos individuais
    ModelID %in% c(55:62) ~ "Individual", 
    
    # Grupo 5: Todas as outras interações complexas
    ModelID %in% c(63:71) ~ "Todas_interacoes",
  ...
  Description = case_when(
    ...
    # Bloco 10: (Item 11.5) Modelos com todas as interações
    ModelID == 63 ~ "E + W + G + GE + GW",
    ModelID == 64 ~ "E + W + P + PE + PW",
    ModelID == 65 ~ "E + W + G + P + GE + PE + GW + PW",
    ModelID == 66 ~ "E + W + GGK + GGKE + GGKW",
    ModelID == 67 ~ "E + W + PGK + PGKE + PGKW",
    ModelID == 68 ~ "E + W + GGK + PGK + GGKE + PGKE + GGKW + PGKW",
    ModelID == 69 ~ "E + W + GAK + GAKE + GAKW",
    ModelID == 70 ~ "E + W + PAK + PAKE + PAKW",
    ModelID == 71 ~ "E + W + GAK + PAK + GAKE + PAKE + GAKW + PAKW",
...
# -------------------------------------------------------------------
# GRUPO 5: MODELOS COMPLETOS COM TODAS AS INTERAÇÕES
# (Modelos Eta65 a Eta72)
# -------------------------------------------------------------------

# Modelo 65: E + W + G + GE + GW (Linear)
Eta65 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  G = list(K = ZG, model = "RKHS"),
...
# Modelo 67: E + W + G + P + GE + PE + GW + PW (Linear)
Eta67 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  G = list(K = ZG, model = "RKHS"),
...
# Modelo 68: E + W + GGK + GGKE + GGKW (Gaussiano)
Eta68 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  GGK = list(K = GGK, model = "RKHS"),
...
# Modelo 69: E + W + PGK + PGKE + PGKW (Gaussiano)
Eta69 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  PGK = list(K = PGK, model = "RKHS"),
...
# Modelo 70: E + W + GGK + PGK + GGKE + PGKE + GGKW + PGKW (Gaussiano)
Eta70 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  GGK = list(K = GGK, model = "RKHS"),
...
# Modelo 71: E + W + GAK + GAKE + GAKW (Arc-cosseno)
Eta71 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  GAK = list(K = GAK, model = "RKHS"),
...
# Modelo 72: E + W + PAK + PAKE + PAKW (Arc-cosseno)
Eta72 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  PAK = list(K = PAK, model = "RKHS"),
...
# Modelo 73: E + W + GAK + PAK + GAKE + PAKE + GAKW + PAKW (Arc-cosseno)
Eta73 <- list(
  E = list(X = ZE, model = "BRR"),
  W = list(K = ZW, model = "RKHS"),
  GAK = list(K = GAK, model = "RKHS"),
...
length(Climate_Models)  # Deve retornar 36

# 2. Adicionamos os modelos de efeitos individuais (Grupo 4)
Climate_Models <- c(Climate_Models,
                    list(Eta57, Eta58, Eta59, Eta60, Eta61, Eta62, Eta63,
                         Eta64, Eta65, Eta66, Eta67, Eta68, Eta69, Eta70, 
                         Eta71, Eta72, Eta73))

length(Climate_Models)  # Deve retornar 53
...
# de validação cruzada, permitindo uma comparação completa de todos os 71 modelos.
...
### 13.0) Visualização dos esquemas de validação cruzada
...
# Parâmetros gerais
# ------------------------------------------------------------
n_geno <- 50  # número de genótipos
n_env  <- 4
envs <- paste0("Env ", 1:n_env)

# ------------------------------------------------------------
# Função para gerar estrutura de cada esquema CV
# ------------------------------------------------------------
generate_cv_data <- function(type) {
  ...
  if (type == "CV2") {
    # 20% dos genótipos testados (genótipo parcialmente testado)
    test_genos <- sample(1:nrow(df), nrow(df) * 0.2)
    df$Test[test_genos] <- TRUE
    
    
  } else if (type == "CV1") {
    # 20% dos genótipos totalmente não vistos
    test_genos <- sample(1:n_geno, n_geno * 0.2)
    df$Test[df$Genotype %in% test_genos] <- TRUE
    
  } else if (type == "CV0") {
    # Um ambiente completamente novo (Env 2)
    df$Test[df$Env == "Env 2"] <- TRUE

    
  } else if (type == "CV00") {
    # Genótipos e ambiente totalmente novos (sem esses genótipos em outros ambientes)
    test_genos <- sample(1:n_geno, n_geno * 0.2)
    df$Test <- FALSE
    df <- df %>% 
      mutate(Test = ifelse(Env == "Env 2" & Genotype %in% test_genos, TRUE, FALSE)) %>%
      filter(!(Genotype %in% test_genos & Env != "Env 2")) %>% # remove genótipos de teste dos outros ambientes
      filter(!(Env == "Env 2" & !(Genotype %in% test_genos))) # mantém apenas genótipos de teste em Env 2
  }
  
  # garantir ordem correta dos ambientes
  df$CV <- type
  df
}
...
# ------------------------------------------------------------
# Função para criar gráfico individual
# ------------------------------------------------------------
plot_cv <- function(df, title) {
  ggplot(df, aes(x = as.character(Env), y = Genotype)) +
    ...
    # Pontos pretos para observações de teste
...
# ------------------------------------------------------------
# Combinar gráficos com eixos globais
# ------------------------------------------------------------
...
# (Esta função não precisa de alteração)
combos <- expand.grid(MODEL = 66:length(Models_full_set), REP = seq_len(num_rep), TRAIT = c("GY", "KW"))
```
        ...
        .packages = c("BGLR", "dplyr", "plyr")) %dopar% {
          ...
          for (env_idx in seq_along(envs_leaveout)) {
            ...
            write.csv(CV00out,
                      file = file.path(
                        "output/results",
                        paste0("CV00_", TRAIT, "_", env_name, "_Eta", MODEL, "_rep", rep_num, ".csv")
                      ),
                      row.names = FALSE)
            write.csv(CV0out,
                      file = file.path(
                        "output/results",
                        paste0("CV0_", TRAIT, "_", env_name, "_Eta", MODEL, "_rep", rep_num, ".csv")
                      ),
                      row.names = FALSE)
          }
...
p <- df_cor %>% 
  ...
  filter(ModelID %in% c(7:12, 16:18, 63:71)) %>% 
  droplevels() %>% 
  ...
  theme_bw() +
  facet_grid(Env_Val ~  Trait) +  # Um painel para cada esquema de validação
...
ggsave("output/Pred.ability/grafico_exemplo_CV0.tiff",
       plot = p,
       width = 8,
       height = 8, dpi = 300)
```
...
ggsave("output/Pred.ability/grafico_exemplo_CV00.tiff",
       plot = p,
       width = 8,
       height = 8, dpi = 300)
```
...
p <- df_cor %>% 
  ...
  filter(ModelID %in% c(7:12, 16:18, 63:71)) %>% 
  droplevels() %>% 
  ggplot(aes(x = reorder(Description, M), y = M, fill = factor(Type))) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.8, show.legend = FALSE) +
  geom_errorbar(aes(ymin = M, ymax = M + SD),
                width = .2,
                position = position_dodge(.9)) +
