...
## 15. Texto automatizado orientado aos resultados

Os parágrafos abaixo são gerados diretamente a partir dos objetos-resumo criados neste script. Sempre que as saídas de predição forem atualizadas e o arquivo for renderizado novamente, esse texto será automaticamente reescrito com base nos resultados atuais.

```{r automated-results-text-pt, results='asis'}
fmt3 <- function(x) sprintf("%.3f", round(x, 3))

trait_labels <- c(
  "GY" = "Produtividade de grãos (GY)",
  "KW" = "Peso de grãos (KW)"
)
...
for (current_trait in traits_to_report) {
  ...
  for (current_cv in c("CV1", "CV2")) {
    ...
    cat(
      "Para **", current_trait, "** em **", current_cv, "**, o modelo com melhor desempenho foi **",
    ...
    if (nrow(best_models) >= 2) {
      cat(
        ", seguido por **", best_models$FullEffects[2], "** (",
    ...
    cat(
      ". Os menores valores neste cenário foram observados para **",
  ...
  for (current_cv in c("CV0", "CV00")) {
    ...
    cat(
      "Em **", current_cv, "**, os modelos com melhor desempenho por ambiente deixado de fora foram ",
      paste0(
        "**", best_by_env$FullEffects, "** em **", best_by_env$Env_leave,
        "** (", fmt3(best_by_env$MeanCorrelation), ")",
        collapse = "; "
      ),
      ". Entre os ambientes, os menores valores variaram de ",
  ...
  if (nrow(current_w) > 0) {
    cat(
      "Nos contrastes pareados, o efeito médio da adição de **W** foi ",
      paste0(
        "**", current_w$CV, "**: ",
  ...
  if (nrow(current_kernel) > 0) {
    cat(
      "Nos contrastes de famílias de kernel dentro das vias analíticas, a diferença média Gaussiano-menos-Linear foi ",
      fmt3(mean(current_kernel$Delta_Gaussian_vs_Linear, na.rm = TRUE)),
      " e a diferença média Arc-cosine-menos-Linear foi ",
