...
2. interpretar os nomes dos arquivos e recuperar traço, modelo, repetição e a informação de leave-one-environment-out;
...
Este chunk define as opções globais de renderização do documento para que tabelas e figuras apareçam com um padrão de apresentação consistente.
...
Este chunk carrega os pacotes de visualização e manipulação de dados, localiza as pastas do projeto e prepara os diretórios de saída usados neste módulo.
...
Este chunk define o catálogo reduzido de modelos usado para anotar resumos, figuras e tabelas comparativas ao longo do documento.
...
Este chunk especifica os pares de modelos comparados nos contrastes entre versões sem `W` e com `W` usados mais adiante no tutorial.
...
Este chunk carrega silenciosamente as tabelas já produzidas pelo arquivo em inglês, evitando repetir a leitura e a consolidação completas dos resultados de predição.
...
Na versão funcional em português, os blocos de leitura, consolidação e exportação permanecem documentados, mas não são reexecutados durante o knit. Em vez disso, este arquivo carrega silenciosamente as tabelas já produzidas pela versão em inglês de `visualization.Rmd`, preservando as tabelas e figuras finais sem duplicar o processamento.
...
Este chunk documenta a localização dos arquivos de predição gerados no módulo anterior e a separação deles por cenário de validação.
...
Este chunk documenta a leitura dos arquivos de CV1 e CV2, incluindo a interpretação dos nomes e a reconstrução dos metadados usados nos resumos.
...
for (i in seq_along(cv12_files)) {
  ...
  if (length(parts) != 4) {
    stop( "Unexpected filename for CV1/CV2 file: ",
     file_name,
  ...
  if (!(".id" %in% names(current_data)) || !("Env" %in% names(current_data)) || !("cor" %in% names(current_data))) {
    stop( "Unexpected column structure in file: ",
     file_name,
...
Este chunk documenta a leitura dos arquivos de CV0 e CV00, incluindo a identificação explícita do ambiente deixado de fora.
...
for (i in seq_along(cvloo_files)) {
  ...
  parts <- strsplit(file_name, "_")[[1]]
  if (length(parts) != 6) {
    stop( "Unexpected filename for CV0/CV00 file: ",
     file_name,
      ".\nExpected pattern CV0_<trait>_<env1>_<env2>_<model>_rep<rep>.csv or CV00_<trait>_<env1>_<env2>_<model>_rep<rep>.csv" )
  }
  current_cv <- parts[1]
  current_trait <- parts[2]
