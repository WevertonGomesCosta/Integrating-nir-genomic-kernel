...
cv12_summary <- cv12_summary %>%
  dplyr::mutate(
    ...
    Group = dplyr::case_when(
      Pathway == "Genomic pathway" ~ "Genômico",
      Pathway == "Phenomic pathway" ~ "Fenômico",
      Pathway == "Combined G + P pathway" ~ "Combinado",
...
cv12_plot_data <- cv12_summary %>%
  mutate(
    ...
    GroupDisplay = dplyr::case_when(
      Pathway == "Genomic pathway" ~ "Genômico",
      Pathway == "Phenomic pathway" ~ "Fenômico",
      Pathway == "Combined G + P pathway" ~ "Combinado",
      TRUE ~ as.character(Group)
    ),
    GroupDisplay = factor(GroupDisplay, levels = group_levels_pt)
  )
...
for (current_trait in cv12_traits) {
  for (current_cv in cv12_schemes) {
    ...
    p_trait_cv12 <- ggplot(
      trait_cv_data,
      aes(x = FullEffectsWrapped, y = MeanCorrelation, fill = GroupDisplay)
    ) +
