...
## 4. Tabelas-resumo de predição

### 4.1 Reconstrução opcional a partir dos resultados em nível de fold

A versão em inglês documenta o bloco mecânico que reconstrói `prediction_cv1_cv2_summary.csv` e `prediction_cv0_cv00_summary.csv` diretamente dos arquivos em `output/results/`. Como esses resultados em nível de fold são regeneráveis e não são versionados, a versão em português reutiliza as tabelas-resumo versionadas e evita repetir essa consolidação.

### 4.2 Carregar as tabelas-resumo versionadas
