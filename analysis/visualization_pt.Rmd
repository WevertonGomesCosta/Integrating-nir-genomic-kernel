...
translate_visual_labels <- function(df) {
  if ("Pathway" %in% names(df)) {
    df$Pathway <- dplyr::recode(
      df$Pathway,
  ...
  if ("KernelFamily" %in% names(df)) {
    df$KernelFamily <- dplyr::recode(
      df$KernelFamily,
      "Gaussian" = "Gaussiano"
    )
  }
  if ("WeatherStatus" %in% names(df)) {
    df$WeatherStatus <- dplyr::recode(
      df$WeatherStatus,
      "Sem W" = "Sem W",
      "Com W" = "Com W"
    )
  ...
  if ("Group" %in% names(df)) {
    df$Group <- dplyr::recode(
      df$Group,
      "Base genômica" = "Base genômica",
      "Base fenômica" = "Base fenômica",
      "Base combinada" = "Base combinada",
      "Clima fonte única" = "Clima fonte única",
      "Clima combinado" = "Clima combinado"
    )
  }
  if ("Description" %in% names(df)) {
    df$Description <- dplyr::recode(
      df$Description,
      "Genômico linear + E" = "Genômico linear + E",
      "Genômico linear + W" = "Genômico linear + W",
      "Genômico Gaussiano + E" = "Genômico Gaussiano + E",
      "Genômico Gaussiano + W" = "Genômico Gaussiano + W",
      "Genômico arc-cosine + E" = "Genômico arc-cosine + E",
      "Genômico arc-cosine + W" = "Genômico arc-cosine + W",
      "Fenômico linear + E" = "Fenômico linear + E",
      "Fenômico linear + W" = "Fenômico linear + W",
      "Fenômico Gaussiano + E" = "Fenômico Gaussiano + E",
      "Fenômico Gaussiano + W" = "Fenômico Gaussiano + W",
      "Fenômico arc-cosine + E" = "Fenômico arc-cosine + E",
      "Fenômico arc-cosine + W" = "Fenômico arc-cosine + W",
      "Combinado linear + E" = "Combinado linear + E",
      "Combinado linear + W" = "Combinado linear + W",
      "Combinado Gaussiano + E" = "Combinado Gaussiano + E",
      "Combinado Gaussiano + W" = "Combinado Gaussiano + W",
      "Combinado arc-cosine + E" = "Combinado arc-cosine + E",
      "Combinado arc-cosine + W" = "Combinado arc-cosine + W"
    )
  }
  if ("Comparison" %in% names(df)) {
    df$Comparison <- dplyr::recode(
      df$Comparison,
...
model_catalog <- translate_visual_labels(model_catalog)
cv12_summary <- translate_visual_labels(cv12_summary)
cvloo_summary <- translate_visual_labels(cvloo_summary)
cv12_w_effect <- translate_visual_labels(cv12_w_effect)
cvloo_w_effect <- translate_visual_labels(cvloo_w_effect)
cv12_kernel_family_compare <- translate_visual_labels(cv12_kernel_family_compare)
cvloo_kernel_family_compare <- translate_visual_labels(cvloo_kernel_family_compare)
cv12_table_display <- translate_visual_labels(cv12_table_display)
cvloo_table_display <- translate_visual_labels(cvloo_table_display)
cv12_top3 <- translate_visual_labels(cv12_top3)
cvloo_top3 <- translate_visual_labels(cvloo_top3)
```
...
```{r table-cv12-top3}
cv12_top3 %>%
  knitr::kable( caption = "Top 3 modelos para cada trait e esquema de validação em CV1 e CV2.",
    align = "llccc" ) %>%
...
```{r table-cvloo-top3}
cvloo_top3 %>%
  knitr::kable( caption = "Top 3 modelos para cada trait, esquema de validação e ambiente deixado de fora em CV0 e CV00.",
    align = "lllccc" ) %>%
  kableExtra::kable_styling( bootstrap_options = c("striped", "hover", "condensed", "responsive"),
   full_width = FALSE,
    position = "left" )
