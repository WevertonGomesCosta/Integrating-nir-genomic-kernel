...
base_prediction_plot <- function(plot_data, x_label, y_label, legend_title) {
  ggplot(plot_data, aes(x = Model, y = MeanCorrelation, fill = Group)) +
...
plot_gy_cv1 <- cv12_plot_data %>%
  ...
  base_prediction_plot("Modelo", "Correlação preditiva", "Grupo de comparação") +
  labs(title = "CV1")
...
prediction_GY_CV1_CV2_combined <- (plot_gy_cv1 + plot_gy_cv2) +
  ...
  patchwork::plot_annotation(
    title = "Capacidade preditiva para produtividade de grãos (GY)",
    tag_levels = "A"
  ) &
  theme(
    legend.position = "top",
...
prediction_KW_CV1_CV2_combined <- (plot_kw_cv1 + plot_kw_cv2) +
  patchwork::plot_layout(ncol = 2, guides = "collect") +
  patchwork::plot_annotation(
    title = "Capacidade preditiva para peso de grãos (KW)",
    tag_levels = "A"
  ) &
  theme(
    legend.position = "top",
