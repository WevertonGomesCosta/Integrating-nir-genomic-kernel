...
Este chunk inicia uma sessão limpa e define a raiz do projeto usada pelo módulo de visualização. O
objetivo é tornar reprodutível desde o início a leitura e o resumo das saídas de predição.
...
Este chunk carrega os pacotes necessários para leitura, resumo e visualização dos resultados de
predição. Ele também define as pastas onde ficam as saídas brutas e as tabelas derivadas.
...
Este chunk recria um catálogo compacto de modelos para que os resumos e figuras possam ser
interpretados com rótulos estáveis. Manter o catálogo próximo da etapa de visualização facilita
acompanhar as comparações.
...
Este chunk define os pares de modelos e as famílias que serão contrastados nas tabelas-resumo. Esses
agrupamentos fornecem a estrutura analítica usada para interpretar o desempenho preditivo.
...
Este chunk restaura as tabelas e objetos de visualização já gerados para que a versão em português
possa enfatizar a apresentação e a interpretação dos resultados finais. Assim, o documento permanece
alinhado ao fluxo principal sem repetir toda a leitura das saídas brutas.
...
Este chunk varre as pastas de resultados e registra os arquivos CSV disponíveis para a etapa de
visualização. O inventário resultante define exatamente quais saídas de predição serão resumidas.
...
if (length(cv12_files) == 0 && length(cvloo_files) == 0) {
  stop(
    "Nenhum arquivo CSV de predição foi encontrado em output/results/. ",
...
Este chunk lê os arquivos de resultados de CV1 e CV2 e converte seus caminhos em identificadores
estruturados, como trait, repetição, fold e modelo. Essa transformação é essencial para as
...
for (i in seq_along(cv12_files)) {
  ...
  if (length(path_parts) != 5) {
    stop(
      ...
      current_rel_path,
      ". Padrão esperado: CV1/<trait>/repXX/foldXX/<file>.csv ou ",
  ...
  if (length(file_parts) != 5) {
    stop(
      ...
      file_name,
      ". Padrão esperado: CV1_<trait>_Eta<model>_repXX_foldXX.csv ou ",
...
Este chunk lê os arquivos de resultados de CV0 e CV00 e extrai os identificadores sensíveis ao
ambiente guardados nos caminhos e nomes dos arquivos. O resultado fornece a tabela em formato longo
usada nos resumos leave-one-environment-out.
...
for (i in seq_along(cvloo_files)) {
  ...
  if (length(path_parts) != 6) {
    stop(
      ...
      current_rel_path,
      ". Padrão esperado: CV0/<Env_leave>/<trait>/repXX/foldXX/<file>.csv ou ",
  ...
  if (length(file_parts) < 6) {
    stop(
      ...
      file_name,
      ". Padrão esperado: CV0_<trait>_<Env_leave>_Eta<model>_repXX_foldXX.csv ou ",
...
Este chunk agrega os resultados de CV1 e CV2 em resumos por modelo. Essas tabelas quantificam o
comportamento preditivo médio ao longo de repetições e folds nos cenários de partição genotípica.
...
Este chunk agrega os resultados de CV0 e CV00 em resumos sensíveis ao ambiente. O objetivo é
descrever o comportamento da predição quando o problema de validação se centra na extrapolação entre
ambientes.
...
Este chunk constrói as tabelas de comparação derivadas que sustentam o ranking e a interpretação dos
modelos. Essas tabelas traduzem os resumos brutos em contrastes analíticos mais focados.
...
Este chunk formata e salva as principais tabelas usadas posteriormente no relatório. Ele oferece uma
apresentação legível dos resumos numéricos antes da construção das figuras.
...
Este chunk apresenta os modelos de melhor desempenho nos cenários CV1 e CV2. O objetivo é destacar
os principais candidatos sob validação por partição de genótipos.
...
Este chunk apresenta os modelos de melhor desempenho nos cenários CV0 e CV00. O foco é destacar os
melhores desempenhos sob validação leave-one-environment-out.
...
Este chunk resume como a inclusão do efeito climático altera o desempenho em CV1 e CV2. A comparação
ajuda a interpretar a contribuição das extensões de modelo informadas por clima.
...
Este chunk resume como a inclusão do efeito climático altera o desempenho em CV0 e CV00. Essa
comparação é especialmente informativa para entender a predição entre ambientes.
...
Este chunk compara as principais famílias de kernel nos resumos de CV1 e CV2. Ele ajuda a distinguir
se estruturas lineares, Gaussianas ou arc-cosine tendem a performar melhor nesses cenários.
...
Este chunk compara as principais famílias de kernel nos resumos de CV0 e CV00. O objetivo é entender
se a estrutura de kernel preferida muda quando o problema de predição se torna ambientalmente mais
rigoroso.
...
Este chunk cria a figura que resume o desempenho dos modelos em CV1 e CV2. O gráfico oferece um
ranking visual compacto para os cenários de partição genotípica.
...
Este chunk cria a figura que resume o desempenho dos modelos em CV0 e CV00. O gráfico destaca como o
ranking dos modelos muda sob validação leave-one-environment-out.
