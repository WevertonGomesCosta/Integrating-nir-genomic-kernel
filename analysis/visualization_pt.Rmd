...
Este arquivo é a **versão didática** do fluxo final de visualização. Sua função é ler as saídas de predição geradas por `analysis_prediction_run_outputs.Rmd`, reorganizá-las em tabelas-resumo tidy e depois construir as figuras e os displays compactos usados adiante no manuscrito.

O fluxo atual de predição grava arquivos brutos de saída em `output/results/` usando os seguintes padrões:
...
Neste fluxo, a repetição e o fold são recuperados diretamente do nome do arquivo. Para `CV0` e `CV00`, o rótulo do ambiente deixado de fora também é recuperado do nome e depois associado a `Env` pelo filtro `Env == Env_leave`.
...
A próxima etapa lê os arquivos `CV1` e `CV2`, interpreta o nome do arquivo e recompõe os metadados necessários para os resumos posteriores. Neste fluxo, a repetição e o fold são recuperados diretamente do nome do arquivo.
...
for (i in seq_along(cv12_files)) {
  ...
  if (length(parts) != 5) {
    stop(
      ...
      file_name,
      ".
Padrão esperado: CV1_<trait>_<model>_rep<rep>_fold<fold>.csv ou CV2_<trait>_<model>_rep<rep>_fold<fold>.csv"
    )
  }
...
A lógica é análoga à seção anterior, mas agora o nome do arquivo também carrega o rótulo do ambiente deixado de fora. No fluxo final, esse rótulo pode conter múltiplos underscores, então o parser reconstrói `Env_leave` a partir do nome do arquivo lendo da direita para a esquerda.
...
for (i in seq_along(cvloo_files)) {
  ...
  if (length(parts) < 6) {
    stop(
      ...
      file_name,
      ".
Padrão esperado: CV0_<trait>_<Env_leave>_<model>_rep<rep>_fold<fold>.csv ou CV00_<trait>_<Env_leave>_<model>_rep<rep>_fold<fold>.csv"
    )
  }
...
- a repetição e o fold estão codificados no nome do arquivo;
- `CV0` e `CV00` carregam no nome o rótulo do ambiente deixado de fora;
