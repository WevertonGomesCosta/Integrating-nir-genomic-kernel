...
write_csv(plot_data, "output/componentes_variancia/Variance.Components.Percentage.PlotData.csv")
```
...
library(patchwork) # Garante que a biblioteca está carregada

# --- Gráfico A: Grain Yield (GY) ---
# O mais importante: legend.position = "none"
g_gy <- plot_data %>%
filter(Trait == "GY") %>%
...
geom_col(position = "fill", color = "black", alpha = 0.85, width = 0.8) +
labs(
 title = NULL, # Título principal será adicionado depois
 subtitle = "GY (Grain Yield)", # Subtítulo específico do painel
...
theme_minimal() +
theme(
 legend.position = "none", # **CRÍTICO: Remove a legenda deste gráfico**
...
# --- Gráfico B: Kernel Weight (KW) ---
# O mais importante: A legenda é definida AQUI
g_kw <- plot_data %>%
filter(Trait == "KW") %>%
ggplot(aes(x = reorder(Description, ModelID), y = Percentage, fill = Component)) +
geom_col(position = "fill", color = "black", alpha = 0.85, width = 0.8) +
labs(
 title = NULL,
 subtitle = "KW (Kernel Weight)",
 x = "Predictive Model (Description)", # Mantém o rótulo X
 y = "Percentage of Total Variance",
 fill = "Component:"
) +
scale_y_continuous(labels = scales::percent_format()) +
...
theme_minimal() +
theme(
 # **CRÍTICO: Define a legenda e sua formatação AQUI**
 legend.position = "bottom",
...
# Define o layout da legenda (ex: 3 linhas)
guides(fill = guide_legend(
    ncol = 1, 
    byrow = FALSE, 
    title.position = "top" # Coloca o título acima da legenda
  ))

# --- Combinar os Gráficos ---

# 1. Empilha os gráficos
combined_plot <- g_gy / g_kw

# 2. Adiciona anotações (Títulos e Tags A/B)
# 3. Adiciona o layout para 'coletar' a legenda (que está em g_kw)
final_plot <- combined_plot +
plot_annotation(
 title = "Percentage Contribution of Variance Components",
 tag_levels = 'A', # Adiciona tags 'A' e 'B'
 theme = theme(
   plot.title = element_text(face = "bold"),
   plot.subtitle = element_text(face = "italic")
  )
) +
plot_layout(guides = 'collect') # Coleta a legenda definida em g_kw

# Imprime o gráfico combinado final
print(final_plot)

# (Opcional) Salvar o gráfico combinado
ggsave("output/componentes_variancia/variance_components_percentage_COMBINED.tiff",
 plot = final_plot,
