# article_results_for_manuscript.R
# ------------------------------------------------------------
# Purpose:
# Collect the main numerical results that will be discussed in the article.
...
model_catalog <- tibble(
  ...
  OriginalEta = c(
    "Eta7", "Eta8", "Eta9", "Eta10", "Eta11", "Eta12",
    "Eta16", "Eta17", "Eta18",
    "Eta65", "Eta66", "Eta67", "Eta68", "Eta69", "Eta70", "Eta71", "Eta72", "Eta73"
  ),
  Group = c(
    rep("E + kernel + interaction", 6),
    rep("E + combined kernels + interactions", 3),
    rep("E + W + kernels + interactions", 9)
  ),
  Description = c(
    "E + G + GE",
    "E + P + PE",
...
if (file.exists(file.path(matrices_dir, "ZG.rds"))) {
  ZG <- readRDS(file.path(matrices_dir, "ZG.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "ZG", nrow = nrow(ZG), ncol = ncol(ZG)))
}

if (file.exists(file.path(matrices_dir, "ZP.rds"))) {
  ZP <- readRDS(file.path(matrices_dir, "ZP.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "ZP", nrow = nrow(ZP), ncol = ncol(ZP)))
}

if (file.exists(file.path(matrices_dir, "ZW.rds"))) {
  ZW <- readRDS(file.path(matrices_dir, "ZW.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "ZW", nrow = nrow(ZW), ncol = ncol(ZW)))
}

if (file.exists(file.path(matrices_dir, "ZEZE.rds"))) {
  ZEZE <- readRDS(file.path(matrices_dir, "ZEZE.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "ZEZE", nrow = nrow(ZEZE), ncol = ncol(ZEZE)))
}

if (file.exists(file.path(matrices_dir, "GGK.rds"))) {
  GGK <- readRDS(file.path(matrices_dir, "GGK.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "GGK", nrow = nrow(GGK), ncol = ncol(GGK)))
}

if (file.exists(file.path(matrices_dir, "PGK.rds"))) {
  PGK <- readRDS(file.path(matrices_dir, "PGK.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "PGK", nrow = nrow(PGK), ncol = ncol(PGK)))
}

if (file.exists(file.path(matrices_dir, "GAK.rds"))) {
  GAK <- readRDS(file.path(matrices_dir, "GAK.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "GAK", nrow = nrow(GAK), ncol = ncol(GAK)))
}

if (file.exists(file.path(matrices_dir, "PAK.rds"))) {
  PAK <- readRDS(file.path(matrices_dir, "PAK.rds"))
  kernel_summary <- bind_rows(kernel_summary, tibble(Object = "PAK", nrow = nrow(PAK), ncol = ncol(PAK)))
}
...
# ------------------------------------------------------------
# 7. Read prediction results from output/results
# ------------------------------------------------------------
...
cv12_files <- all_result_files[grepl("CV1_|CV2_", basename(all_result_files))]
cvloo_files <- all_result_files[grepl("CV0_|CV00_", basename(all_result_files))]

if (length(cv12_files) > 0) {
  ...
  for (i in seq_along(cv12_files)) {
    ...
    current_data$ModelNumber <- as.numeric(gsub("Eta", "", parts[3]))
    current_data$Model <- sprintf("M%02d", current_data$ModelNumber[1])
  ...
  cv12_summary <- cv12_raw %>%
    group_by(ModelNumber, Model, OriginalEta, Group, Description, Trait, CV) %>%
    summarise(
      MeanCorrelation = mean(cor, na.rm = TRUE),
      SDCorrelation = sd(cor, na.rm = TRUE),
      .groups = "drop"
    ) %>%
  ...
  weather_pairs <- tibble(
    WithoutW = c("M01", "M02", "M07", "M03", "M04", "M08", "M05", "M06", "M09"),
    WithW = c("M10", "M11", "M12", "M13", "M14", "M15", "M16", "M17", "M18")
  )

  weather_pairs_cv12 <- weather_pairs %>%
    left_join(cv12_summary %>% select(Trait, CV, Model, MeanCorrelation),
              by = c("WithoutW" = "Model")) %>%
    rename(Correlation_WithoutW = MeanCorrelation) %>%
    left_join(cv12_summary %>% select(Trait, CV, Model, MeanCorrelation),
              by = c("WithW" = "Model", "Trait", "CV")) %>%
    rename(Correlation_WithW = MeanCorrelation) %>%
    mutate(Delta_WithW_Minus_WithoutW = Correlation_WithW - Correlation_WithoutW)

  write_csv(weather_pairs_cv12, file.path(article_dir, "prediction_cv1_cv2_weather_delta_pairs.csv"))

  report_lines <- c(
    report_lines,
    "PREDICTIVE ABILITY: CV1 AND CV2",
    paste("Number of CV1/CV2 files processed:", length(cv12_files)),
    "Raw consolidated table: output/article_results/prediction_cv1_cv2_raw.csv",
    ...
    "Weather effect table: output/article_results/prediction_cv1_cv2_weather_delta_pairs.csv",
    ""
  )
  ...
  mean_weather_delta_cv12 <- weather_pairs_cv12 %>%
    group_by(Trait, CV) %>%
    summarise(
      MeanDeltaWithW = mean(Delta_WithW_Minus_WithoutW, na.rm = TRUE),
      .groups = "drop"
    )

  write_csv(mean_weather_delta_cv12, file.path(article_dir, "prediction_cv1_cv2_mean_weather_delta.csv"))

  report_lines <- c(report_lines, "", "Average effect of adding W to matched model pairs (CV1/CV2):")
  ...
  report_lines <- c(
    report_lines,
    "PREDICTIVE ABILITY: CV1 AND CV2",
    "No CV1/CV2 CSV files were found in output/results.",
    ""
  )
}

if (length(cvloo_files) > 0) {
  ...
  for (i in seq_along(cvloo_files)) {
    ...
    current_data$ModelNumber <- as.numeric(gsub("Eta", "", parts[5]))
    current_data$Model <- sprintf("M%02d", current_data$ModelNumber[1])
  ...
  if ("Env" %in% colnames(cvloo_raw)) {
    cvloo_summary <- cvloo_raw %>%
      mutate(Env = as.character(Env), Env_leave = as.character(Env_leave)) %>%
      filter(Env == Env_leave) %>%
      group_by(ModelNumber, Model, OriginalEta, Group, Description, Trait, CV, Env_leave) %>%
      summarise(
        MeanCorrelation = mean(cor, na.rm = TRUE),
        SDCorrelation = sd(cor, na.rm = TRUE),
        .groups = "drop"
  ...
  report_lines <- c(
    ...
    "Raw consolidated table: output/article_results/prediction_cv0_cv00_raw.csv",
  ...
  report_lines <- c(
    report_lines,
    "PREDICTIVE ABILITY: CV0 AND CV00",
    "No CV0/CV00 CSV files were found in output/results.",
    ""
  )
}
...
if (!is.null(variance_data)) {
  ...
  variance_data <- variance_data %>%
    left_join(model_catalog, by = "Model")
...
reviewer_notes <- tibble(
  ...
  What_the_script_provides = c(
    "No numerical output needed. Prefer 'weather' for W when referring only to climatic variables.",
    ...
    "This script does not compute convergence by itself unless full BGLR chains were saved. If chains were not saved, rerun at least selected models keeping full fit objects for trace plots and effective sample size.",
    "The script exports concise prediction and variance tables that can be used to revise figures and captions."
  )
)
