...
A estação experimental estava localizada em 30°33′04.8″ N, 96°26′00.3″ W (aproximadamente 30.55133° N, 96.43342° W). Essas coordenadas são usadas tanto para a consulta à NASA POWER quanto para os cálculos dependentes de coordenadas, como o fotoperíodo.
...
Em seguida, definimos a localização do estudo, os anos e os metadados básicos que controlam quais séries meteorológicas serão usadas e como os resultados serão identificados.

```{r parameters}
# Coordenadas da estação experimental
...
Neste tutorial, `PHOTOPERIOD_HOURS` representa o fotoperíodo diário em horas, calculado a partir das coordenadas da estação experimental, e é preservado como variável intermediária reprodutível. A variável `PTR_DAILY` é a razão entre o fotoperíodo diário e os graus-dia (`GDD`). Ela é calculada apenas quando `GDD > 0`, evitando divisão por zero ou valores sem interpretação biológica. O resumo anual correspondente é `PTR_AVG`.
...
# Os nomes dos arquivos de saída usam "environmental_covariates" para preservar
# a compatibilidade com os scripts posteriores do pipeline do projeto.
...
Os dois últimos arquivos usam os nomes `environmental_covariates` requeridos pelos scripts posteriores e contêm covariáveis derivadas do clima.
