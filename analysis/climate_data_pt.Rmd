...
- `suncalc`: é usado na versão em inglês para calcular nascer e pôr do sol e, a partir deles, o fotoperíodo diário.
...
Neste tutorial, `PHOTOPERIOD_HOURS` representa o fotoperíodo diário em horas e é mantido como variável de auditoria. A variável `PTR_DAILY` é a razão entre o fotoperíodo diário e os graus-dia (`GDD`). Ela é calculada apenas quando `GDD > 0`, evitando divisão por zero ou valores sem interpretação biológica. O resumo anual correspondente é `PTR_AVG`.
