...
Este chunk configura o documento e mantém consistentes as opções gráficas e de relatório ao longo do
módulo. Assim, as etapas seguintes podem se concentrar apenas no fluxo de processamento climático.
...
Este chunk carrega os pacotes necessários para obtenção, reorganização, resumo e visualização dos
dados climáticos. Reunir as dependências aqui torna as etapas seguintes mais fáceis de acompanhar.
...
Este chunk define a janela temporal, a localização e os controles gerais usados para consultar a
NASA POWER e organizar os registros climáticos. Esses parâmetros determinam o escopo dos dados
ambientais descritos no módulo.
...
Este chunk realiza o primeiro bloco de download dos dados climáticos da NASA POWER. Ele estabelece a
base de dados diária bruta que depois será limpa e harmonizada entre as safras.
...
Este chunk completa o download climático para o período restante e combina os resultados em uma
tabela diária unificada. O objetivo é obter um registro contínuo cobrindo as safras estudadas.
...
Este chunk restaura os dados climáticos já obtidos para que a versão em português apresente a mesma
sequência analítica sem depender de um novo download. Dessa forma, a interpretação fica alinhada ao
fluxo principal.
...
Este chunk mostra uma prévia organizada dos registros diários que serão usados nas figuras e nos
resumos seguintes. A tabela ajuda a visualizar a estrutura dos dados antes das agregações.

```{r daily_preview_table_pt, echo=FALSE}
knitr::kable(
  daily_preview_table,
  align = "c",
  caption = "Tabela 1. Exemplo de registros meteorológicos diários recuperados da NASA POWER para os ensaios no Texas."
...
Este chunk define configurações gráficas reutilizadas nas figuras seguintes. Centralizar essas
opções ajuda a manter comparabilidade visual entre as variáveis.
...
Este chunk constrói o primeiro gráfico de precipitação, enfatizando o perfil diário ao longo das
safras. Ele oferece a visão mais ampla da dinâmica de chuva no período estudado.
...
Este chunk reorganiza a precipitação por mês e dia para facilitar a comparação entre anos dentro da
safra. A figura resultante evidencia melhor o posicionamento temporal dos eventos de chuva.
...
Este chunk agrega a precipitação diária em totais mensais. Esse resumo ajuda a destacar padrões
sazonais mais amplos que podem passar despercebidos nos valores diários.
...
Este chunk resume as temperaturas mínima, média e máxima diárias observadas durante as safras. Ele
ajuda a descrever contrastes térmicos que podem influenciar as etapas analíticas posteriores.
...
Este chunk reúne em uma única figura os principais painéis de precipitação e temperatura. A visão
conjunta oferece um resumo compacto das diferenças climáticas entre as safras.
...
Este chunk gera gráficos diários complementares para radiação solar, velocidade do vento e umidade
relativa. Essas variáveis ampliam a caracterização ambiental além de precipitação e temperatura.
...
Este chunk combina as variáveis climáticas adicionais em um resumo multipainel. Apresentar essas
séries em conjunto facilita comparar seu comportamento sazonal de forma rápida.
...
Este chunk deriva as covariáveis climáticas que serão usadas posteriormente nos módulos de matrizes,
kernels e predição. Ele converte as séries diárias brutas em variáveis estruturadas para o pipeline
analítico.
...
Este chunk restaura as saídas climáticas já processadas para que a versão em português possa
continuar a apresentação a partir dos objetos finais. Assim, a ênfase permanece na interpretação do
resultado climático.
...
Este chunk apresenta tabelas-resumo das covariáveis climáticas derivadas. Essas tabelas ajudam a
verificar como os registros diários foram condensados para uso analítico posterior.

```{r climate_covariate_tables_pt, echo=FALSE}
knitr::kable(
  weather_covariates_dictionary,
  align = c("l", "c", "l"),
  caption = "Tabela 2. Covariáveis derivadas do clima usadas para caracterizar as safras e apoiar a construção do kernel climático."
...
Este chunk grava as saídas climáticas processadas para uso downstream. Com isso, o módulo climático
é encerrado e fornece entradas estáveis para as etapas seguintes do workflow.
