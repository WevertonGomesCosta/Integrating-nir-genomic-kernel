...
Este tutorial apresenta o fluxo de trabalho usado para obter e processar dados meteorológicos diários para os experimentos com milho conduzidos no local experimental em Burleson County, Texas, durante as safras de 2011 e 2012.
...
**Local de estudo:** Burleson County, Texas, EUA  
...
```{r parameters}
# Coordenadas do local experimental (Burleson County, TX)
...
```{r plot_precipitation_overview, fig.width=10, fig.height=5.8, fig.align="center", fig.cap="Figura 1. Precipitação diária ao longo das safras de milho de 2011 e 2012 no local experimental em Burleson County, Texas. As barras representam a precipitação diária corrigida obtida da NASA POWER."}
p_precip_daily <- ggplot(
  ...
  labs(
    ...
    subtitle = "Burleson County, Texas, EUA, 2011-2012",
    x = "Mês",
...
```{r plot_monthly_precipitation, fig.width=8.5, fig.height=5.4, fig.align="center", fig.cap="Figura 3. Totais mensais de precipitação durante as safras de milho de 2011 e 2012 no local experimental em Burleson County, Texas."}
...
Neste tutorial, `PHOTOPERIOD_HOURS` representa o fotoperíodo diário em horas e é mantido como variável intermediária para o cálculo dos descritores fototermais. A variável `PTR_DAILY` é a razão entre o fotoperíodo diário e os graus-dia (`GDD`). Ela é calculada apenas quando `GDD > 0`, evitando divisão por zero ou valores sem interpretação biológica. O resumo anual correspondente é `PTR_AVG`.
