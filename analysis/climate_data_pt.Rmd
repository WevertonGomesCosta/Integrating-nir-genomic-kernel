---
title: "Aquisição e Processamento de Dados Climáticos (NASA POWER)"
author: 
...
**Idioma / Language:** [Português](climate_data_pt.html) | [English](climate_data.html)
...
1. Documentar a recuperação dos dados meteorológicos diários da base NASA POWER.
...
**Próxima etapa**
- `matrizes.Rmd` usa as saídas derivadas do clima disponíveis para alinhar a informação ambiental com a estrutura observacional e construir o kernel climático (`W`).
...
- `suncalc`: calcula nascer e pôr do sol e, a partir deles, o fotoperíodo diário.
...
climate_results_dir <- "output/climate_results"
```
...
As coordenadas confirmadas da estação experimental são 30°33′04.8″ N, 96°26′00.3″ W (aproximadamente 30.55133° N, 96.43342° W). Essas coordenadas são usadas no workflow reprodutível atual tanto para a consulta à NASA POWER quanto para os cálculos dependentes de coordenadas, como o fotoperíodo.

Para preservar a proveniência, o workflow arquivado originalmente consultou a NASA POWER em 30.6282° N, 96.3344° W. Durante a revisão do manuscrito, as duas localizações foram consultadas para as mesmas janelas de 2011 e 2012 e retornaram valores diários idênticos para as seis variáveis meteorológicas usadas neste estudo. O recálculo do fotoperíodo e da razão fototérmica nas coordenadas confirmadas produziu apenas diferenças numéricas desprezíveis, e o kernel climático permaneceu inalterado à precisão de máquina. Assim, as coordenadas arquivadas são mantidas abaixo apenas como metadados de proveniência e não são usadas pelo workflow atual.
...
Em seguida, definimos a localização confirmada, o ponto histórico de extração mantido para proveniência, os anos e os metadados básicos que controlam quais séries meteorológicas serão usadas e como os resultados serão identificados.

```{r parameters}
# Coordenadas confirmadas da estação experimental usadas pelo workflow atual
station_coords <- c(lon = -96.43342, lat = 30.55133)

# Ponto histórico da extração NASA POWER, mantido apenas para proveniência
...
## 4. Documentação da recuperação dos dados na NASA POWER

Esta seção documenta a lógica usada para obter os dados climáticos diários da NASA POWER. A consulta é apresentada apenas como referência didática; a base usada neste documento é lida a partir dos arquivos já disponíveis em `output/climate_results/`.
...
### 4.1 Código de consulta dos dados climáticos diários de cada ano

O código abaixo documenta duas requisições independentes ao serviço NASA POWER, uma para 2011 e outra para 2012. Um identificador `year` é adicionado imediatamente após a obtenção para que a origem de cada registro permaneça explícita ao longo de todo o fluxo.
...
### 4.2 Código de organização da base bruta
...
raw_climate_path <- file.path(climate_results_dir, "texas_climate_data_raw.csv")
...
## 5. Análise exploratória e leitura das figuras
...
### 5.1 Documentar os rótulos compartilhados usados nas figuras
...
```{r additional_plots, include=FALSE}
p4 <- ggplot(
  texas_climate_data,
  ...
  labs(
    ...
    x = "Mês",
    y = expression("Radiação solar de ondas curtas (" * MJ ~ m^{-2} ~ day^{-1} * ")"),
...
p5 <- ggplot(
  ...
  labs(
    ...
    subtitle = "A linha tracejada indica a tendência LOESS",
    x = "Mês",
    y = expression("Velocidade do vento (" * m ~ s^{-1} * ")"),
...
Como na subseção anterior, os gráficos individuais são combinados em uma única figura. O layout usa `patchwork::wrap_plots()`, o que evita erros de composição quando os operadores do patchwork não são reconhecidos corretamente durante a renderização.
...
Algumas covariáveis derivadas do clima dependem de quantidades que não são fornecidas diretamente pela NASA POWER. Por isso, primeiro calculamos variáveis intermediárias diárias como graus-dia (GDD), fotoperíodo, razão fototérmica (PTR), amplitude térmica diária, temperatura média diária, pressão de vapor de saturação, pressão de vapor real e déficit de pressão de vapor (VPD).

Neste tutorial, `PHOTOPERIOD_HOURS` representa o fotoperíodo diário em horas, calculado a partir das coordenadas confirmadas da estação experimental, e é preservado como variável intermediária reprodutível. A variável `PTR_DAILY` é a razão entre o fotoperíodo diário e os graus-dia (`GDD`). Ela é calculada apenas quando `GDD > 0`, evitando divisão por zero ou valores sem interpretação biológica. O resumo anual correspondente é `PTR_AVG`.
...
weather_covariates_dictionary <- tibble::tibble(
  ...
  Descrição = c(
    ...
    "Razão fototérmica diária média, calculada como fotoperíodo em horas dividido por GDD",
...
weather_covariates_path <- file.path(climate_results_dir, "environmental_covariates.csv")
ecs_expanded_path <- file.path(climate_results_dir, "environmental_covariates_expanded.csv")
...
- `PTR_AVG` resume a razão fototérmica diária média, combinando duração do dia e acúmulo térmico efetivo.
