...
O código abaixo reconstrói o ambiente de execução quando o arquivo é renderizado isoladamente. Ele respeita o contrato de `analysis_prediction_pt.Rmd` e recria exatamente o conjunto reduzido de 18 objetos ETA definido na Parte I.
...
run_bglr <- function(y2_vector, ETA, nIter, burnIn, thin, save_prefix) {
  ...
  if (missing(save_prefix) || is.null(save_prefix) || !nzchar(save_prefix)) {
    stop("A persistent save_prefix inside output/results/bglr_runs/ must be provided for each BGLR fit.")
  }
...
foreach(i = seq_len(nrow(combos)),
  ...
  for (fold_num in seq_len(num_fold)) {
    ...
    save_prefix_full <- file.path(
      bglr_runs_dir,
      paste0("full_Eta", MODEL, "_", TRAIT, "_rep", rep_num, "_fold", fold_num, "_")
    )
  ...
  for (fold_num in seq_len(num_fold)) {
    ...
    for (env_idx in seq_along(envs_leaveout)) {
      ...
      save_prefix_env <- file.path(
        bglr_runs_dir,
        paste0(env_name, "_", TRAIT, "_Eta", MODEL, "_rep", rep_num, "_fold", fold_num, "_")
      )
...
## 12. Nota sobre artefatos salvos do `BGLR`

Esta etapa de execução agora armazena dois tipos diferentes de arquivos, e vale a pena distingui-los claramente em um material tutorial.

**Artefatos auxiliares do ajuste salvos para diagnóstico.**  
Durante cada chamada de `BGLR(...)`, o argumento `saveAt` aponta para um prefixo persistente dentro de `output/results/bglr_runs/`. Esses arquivos são gerados pelo `BGLR` enquanto o modelo está sendo ajustado. Eles não são os resumos principais dos esquemas de CV, mas agora são preservados porque podem ser necessários depois em análises de convergência ou em outros diagnósticos do ajuste.

**Saídas analíticas finais.**  
Depois que o ajuste termina, o script coleta em memória as predições, o runtime e os resumos dos CVs e então grava os CSVs finais em `output/results/`. Esses CSVs continuam sendo as principais saídas interpretadas nos módulos posteriores de visualização e resumo.

**Por que vale a pena manter as duas camadas de arquivos.**  
Os arquivos armazenados em `output/results/bglr_runs/` documentam o processo de ajuste em si, enquanto os CSVs em `output/results/` resumem os resultados preditivos comparados entre esquemas de CV, características, ambientes e modelos. Manter as duas camadas disponíveis deixa o workflow mais transparente e preserva o material necessário para análises futuras de convergência.

**Interpretação prática para este módulo.**  
Quando este arquivo inventaria saídas, ele inventaria apenas os CSVs finais armazenados diretamente em `output/results/`. A pasta `bglr_runs` é preservada como um arquivo auxiliar dos artefatos de fitting e não é misturada com as tabelas finais de predição.
