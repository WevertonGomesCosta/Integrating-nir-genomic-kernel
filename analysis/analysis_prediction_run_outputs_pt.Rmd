...
run_bglr <- function(y2_vector, ETA, nIter, burnIn, thin, save_prefix) {
  ...
  if (missing(save_prefix) || is.null(save_prefix) || !nzchar(save_prefix)) {
    stop("A persistent save_prefix must be provided to run_bglr().")
  }
...
foreach(i = seq_len(nrow(combos)),
  ...
  for (fold_num in seq_len(num_fold)) {
    ...
    run_dir <- file.path(bglr_runs_dir, "CV1_CV2", TRAIT, rep_label, fold_label, paste0("Eta", MODEL))
    dir.create(run_dir, recursive = TRUE, showWarnings = FALSE)

    save_prefix_full <- file.path(run_dir, "BGLR_")
...
foreach(i = seq_len(nrow(combos)),
  ...
  for (fold_num in seq_len(num_fold)) {
    ...
    for (env_idx in seq_along(envs_leaveout)) {
      ...
      run_dir <- file.path(bglr_runs_dir, "CV0_CV00", env_name, TRAIT, rep_label, fold_label, paste0("Eta", MODEL))
      dir.create(run_dir, recursive = TRUE, showWarnings = FALSE)

      save_prefix_env <- file.path(run_dir, "BGLR_")
...
output_examples <- bind_rows(
  tibble::tibble(
    ExampleType = "CV1/CV2",
    relative_path = c(
      "CV1/GY/rep01/fold03/CV1_GY_Eta1_rep01_fold03.csv",
      "CV2/KW/rep10/fold05/CV2_KW_Eta18_rep10_fold05.csv"
    ),
    Meaning = c(
      "Acurácia preditiva por ambiente para CV1, trait GY, modelo Eta1, repetição 1, fold 3.",
      "Acurácia preditiva por ambiente para CV2, trait KW, modelo Eta18, repetição 10, fold 5."
    )
  ),
  tibble::tibble(
    ExampleType = "CV0/CV00",
    relative_path = c(
      "CV0/CS11_WS/GY/rep01/fold04/CV0_GY_CS11_WS_Eta1_rep01_fold04.csv",
      "CV00/CS12_WW/KW/rep10/fold02/CV00_KW_CS12_WW_Eta18_rep10_fold02.csv"
    ),
    Meaning = c(
      "Acurácia preditiva por ambiente para CV0, trait GY, ambiente retirado CS11_WS, modelo Eta1, repetição 1, fold 4.",
      "Acurácia preditiva por ambiente para CV00, trait KW, ambiente retirado CS12_WW, modelo Eta18, repetição 10, fold 2."
    )
  )
)

write_csv(output_examples, file.path(analysis_tables_dir, "analysis_prediction_output_examples.csv"))
knitr::kable(output_examples, caption = "Exemplos de caminhos relativos produzidos pelo fluxo de predição.", align = "c")
```
