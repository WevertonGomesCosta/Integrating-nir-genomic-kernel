...
- os artefatos persistentes do `BGLR` são salvos nas pastas do ajuste primário (`CV1` e `CV0`), renomeados com rótulos explícitos dos componentes e depois espelhados para as pastas pareadas (`CV2` e `CV00`).
...
Este bloco implementa a validação leave-one-environment-out para o mesmo workflow de 18 modelos. Para cada tarefa, o mapa determinístico de folds é reconstruído com a mesma regra usada em `CV1/CV2`, um ajuste é rodado para cada combinação `env_leave × fold` e as predições são divididas em `CV0` (genótipos de treinamento sob a máscara do ambiente) e `CV00` (genótipos deixados de fora sob a mesma máscara). Assim como no bloco baseado em genótipos, o caminho de execução permanece explícito. Os artefatos são salvos primeiro em `bglr_runs/CV0/...`, renomeados com rótulos explícitos dos componentes e depois espelhados para `bglr_runs/CV00/...`.
