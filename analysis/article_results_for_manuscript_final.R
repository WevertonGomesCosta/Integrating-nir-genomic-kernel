...
# This tutorialized version keeps the same analytical flow of the final script,
# but explains each block more explicitly so the repository remains easier to
# study, audit, and reuse.
#
...
# ------------------------------------------------------------
# 1. Locate the project root and the main folders used downstream
# ------------------------------------------------------------
# The script accepts two common execution contexts:
# - from the project root;
# - from analysis/.
# This keeps the pipeline easier to run from both RStudio and terminal sessions.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 2. Start the manuscript support report
# ------------------------------------------------------------
# The report is written as plain text because it is easy to inspect, quote,
# and paste into manuscript notes or reviewer responses.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 3. Model catalog used in the reduced final pipeline
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# The script first tries the processed phenotype object generated by matrizes.
# If it is absent, it falls back to the raw NIR.csv file when the required
...
# ------------------------------------------------------------
# This block helps manuscript writing by producing a clean table with variable
...
# ------------------------------------------------------------
# 6. Kernel dimensions and Gaussian-kernel scaling information
# ------------------------------------------------------------
# These outputs are useful in Methods and in reviewer responses when describing
...
if (file.exists(file.path(matrices_dir, "q05G.rds"))) {
  ...
  q05_summary <- bind_rows(
    q05_summary,
    tibble(Quantity = "q05G", Value = as.numeric(q05G), Source = "output/Matrizes/q05G.rds")
  )
  ...
  q05_summary <- bind_rows(
    ...
    tibble(
      Quantity = "q05G",
      Value = as.numeric(stats::quantile(DG_proj, probs = 0.05, na.rm = TRUE)),
      Source = "computed from output/Matrizes/DG_proj.rds"
    )
  )
}
...
if (file.exists(file.path(matrices_dir, "q05P.rds"))) {
  ...
  q05_summary <- bind_rows(
    q05_summary,
    tibble(Quantity = "q05P", Value = as.numeric(q05P), Source = "output/Matrizes/q05P.rds")
  )
  ...
  q05_summary <- bind_rows(
    ...
    tibble(
      Quantity = "q05P",
      Value = as.numeric(stats::quantile(DP_proj, probs = 0.05, na.rm = TRUE)),
      Source = "computed from output/Matrizes/DP_proj.rds"
    )
  )
}
...
# ------------------------------------------------------------
# Preferred source:
#   final summary tables already exported by visualization.Rmd.
# Fallback source:
#   raw CSV files in output/results/.
# This preserves the same logic of the final operational script while making
...
# ------------------------------------------------------------
# The script accepts either the processed CSV or the full RDS object, and then
...
if (!is.null(variance_data)) {
  ...
  if ("Model" %in% colnames(variance_data)) {
    ...
    variance_data$Model <- ifelse(
      grepl("^Eta", variance_data$Model),
...
# ------------------------------------------------------------
# This small table is intentionally simple. It helps map reviewer requests to
# the numerical outputs already generated by the pipeline.
# ------------------------------------------------------------
...
# 10. Final write-out
# ------------------------------------------------------------
# The report is saved as plain text and also printed to the console.
# ------------------------------------------------------------
