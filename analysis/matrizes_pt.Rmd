...
**Idioma / Language:** [Português](matrizes_pt.html) | [English](matrizes.html)
...
Os valores dos marcadores são padronizados **dentro de cada bloco de observações específico de ambiente** antes do empilhamento dos blocos, preservando o pré-processamento utilizado nas análises ajustadas.
...
O ajuste dos kernels não lineares documentado abaixo usa a resposta completa de GY antes da validação cruzada. As configurações Gaussianas e as profundidades de recursão arccosine selecionadas são então fixadas e reutilizadas nas etapas posteriores, inclusive para KW. Consequentemente, o ajuste dos kernels não lineares não é aninhado à validação cruzada, e as comparações resultantes devem ser interpretadas condicionalmente a esse procedimento de ajuste.
