...
Os resumos anuais agora incluem `PTR_AVG`, que representa a razão fototérmica diária média. O arquivo expandido também preserva `PHOTOPERIOD_HOURS` como variável de auditoria e `PTR_DAILY` como descritor climático diário. Como `PHOTOPERIOD_HOURS` é um componente intermediário usado para calcular PTR, ele é excluído manualmente do kernel climático diário, enquanto `PTR_DAILY` permanece elegível para entrar em `ZW`.
...
# PHOTOPERIOD_HOURS é preservado para auditoria, mas não entra
# diretamente em ZW porque PTR_DAILY já combina fotoperíodo e GDD.
