...
Este chunk define as opções de renderização do documento para que código, mensagens e figuras sigam um padrão visual único.
...
Este chunk carrega os pacotes necessários para construção de matrizes, manipulação de dados, cálculo de kernels, otimização e visualização.
...
Este chunk lê os dados NIR e prepara a entrada fenômica que depois será alinhada aos registros fenotípicos e genômicos.
...
Este chunk lê os dados de marcadores genômicos que formam a base dos kernels genômicos usados no pipeline.
...
Este chunk monta a tabela fenotípica que funciona como espinha dorsal, em nível de observação, para as matrizes alinhadas.
...
Este chunk harmoniza os identificadores de genótipo entre as diferentes fontes para garantir que as mesmas unidades sejam comparadas corretamente.
...
Este chunk organiza os dados harmonizados por ambiente, preparando as verificações posteriores de balanceamento, sobreposição e construção dos kernels.
...
Este chunk identifica os genótipos compartilhados entre ambientes, passo importante para a estabilidade do alinhamento entre ambientes.
...
Este chunk constrói o kernel genômico linear que representa a relação genômica básica entre as observações.
...
Este chunk constrói o kernel fenômico linear a partir dos dados NIR para compará-lo às estruturas genômicas e climáticas.
...
Este chunk constrói a matriz categórica de ambiente, separando explicitamente o efeito macroambiental da informação derivada do clima.
...
Este chunk lê as saídas climáticas produzidas pelo módulo anterior para que o kernel climático seja construído a partir de arquivos já salvos.
...
Este chunk padroniza as covariáveis climáticas para colocar as variáveis meteorológicas em escala comparável antes da construção do kernel.
...
Este chunk deriva as assinaturas climáticas por ano que resumem a informação padronizada e servem de base para o kernel climático.
...
Este chunk constrói o kernel climático `W`, que complementa a estrutura categórica de ambiente em vez de substituí-la.
...
Este chunk carrega silenciosamente as matrizes e kernels já produzidos pelo arquivo em inglês, evitando repetir a etapa pesada de construção.
...
Este chunk resume os kernels lineares básicos para que o leitor confira dimensões, amplitudes e comportamento geral antes dos kernels não lineares.
...
Este chunk realiza verificações pontuais em entradas selecionadas para confirmar que as matrizes alinhadas estão se comportando como esperado.
...
Este chunk executa testes explícitos de alinhamento entre matrizes e metadados para detectar incompatibilidades antes dos kernels não lineares.
...
Este chunk constrói os kernels lineares de interação que combinam informação genômica ou fenômica com ambiente e clima.
...
Este chunk define as funções matemáticas necessárias para construir e otimizar os kernels Gaussianos.
...
Este chunk calcula os valores candidatos do kernel Gaussiano genômico antes da escolha final do parâmetro de largura.
...
Este chunk busca o parâmetro do kernel Gaussiano genômico que melhor atende ao comportamento desejado.
...
Este chunk lê e inspeciona a solução escolhida para o kernel Gaussiano genômico antes da construção final do objeto.

```{r ggk-read-solution, include=FALSE, eval=FALSE}
solG <- readRDS("output/Matrizes/solG.RDS")
```

Este chunk constrói o kernel Gaussiano genômico final usando o parâmetro selecionado.
...
Este chunk calcula os valores candidatos do kernel Gaussiano fenômico antes da definição final da largura.
...
Este chunk busca o parâmetro do kernel Gaussiano fenômico usado na definição do objeto final.
...
Este chunk lê e verifica a solução escolhida para o kernel Gaussiano fenômico antes da construção final.

```{r pgk-read-solution, include=FALSE, eval=FALSE}
solP <- readRDS("output/Matrizes/solP.RDS")
```

Este chunk constrói o kernel Gaussiano fenômico final com a largura escolhida.
...
Este chunk gera as estruturas de interação associadas aos kernels Gaussianos para combiná-las com ambiente e clima.
...
Este chunk define as funções matemáticas necessárias para construir os kernels arc-cosine.
...
Este chunk constrói o kernel arc-cosine genômico usado na via genômica não linear.
...
Este chunk verifica o kernel arc-cosine genômico para confirmar que o objeto final está numericamente consistente.
...
Este chunk constrói o kernel arc-cosine fenômico usado na via fenômica não linear.
...
Este chunk verifica o kernel arc-cosine fenômico para confirmar que a matriz final está coerente.
...
Este chunk constrói os kernels de interação associados às estruturas arc-cosine para uso nos modelos posteriores.
...
Este chunk resume os kernels não lineares e suas interações, oferecendo uma visão diagnóstica final desses objetos.
...
Este chunk desenha a figura diagnóstica que compara visualmente o comportamento dos principais kernels.
...
Este chunk grava todas as matrizes e kernels em disco e faz uma limpeza básica para que os módulos seguintes possam carregar os artefatos diretamente.
