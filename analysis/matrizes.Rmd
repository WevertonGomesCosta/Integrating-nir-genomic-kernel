...
```{r ak-functions}
build_arc_cosine_base_kernel <- function(X) {
  cosine_similarity <- cor(t(X))
  cosine_similarity[cosine_similarity > 1] <- 1
  cosine_similarity[cosine_similarity < -1] <- -1

  angle_matrix <- acos(cosine_similarity)
  observation_norm <- sqrt(apply(X, 1, function(x) crossprod(x)))
  norm_crossproduct <- tcrossprod(observation_norm)

  base_kernel <- (1 / pi) * norm_crossproduct *
    (sin(angle_matrix) + (pi - angle_matrix) * cosine_similarity)

  base_kernel <- base_kernel / median(base_kernel, na.rm = TRUE)
  colnames(base_kernel) <- rownames(X)
  rownames(base_kernel) <- rownames(X)
  base_kernel
}

build_arc_cosine_kernel <- function(base_kernel, nl) {
  n <- nrow(base_kernel)
  updated_kernel <- base_kernel

  for (layer_idx in seq_len(nl)) {
    diagonal_crossproduct <- tcrossprod(diag(updated_kernel))
    cosine_similarity <- updated_kernel * (diagonal_crossproduct^(-1 / 2))
    cosine_vector <- as.vector(cosine_similarity)
    cosine_vector[cosine_vector > 1] <- 1
    cosine_vector[cosine_vector < -1] <- -1

    angle_matrix <- matrix(acos(cosine_vector), n, n)

    updated_kernel <- (1 / pi) * (diagonal_crossproduct^(1 / 2)) *
      (sin(angle_matrix) + (pi - angle_matrix) * cos(angle_matrix))
  }

  updated_kernel <- updated_kernel / median(updated_kernel, na.rm = TRUE)
  rownames(updated_kernel) <- rownames(base_kernel)
  colnames(updated_kernel) <- colnames(base_kernel)
  updated_kernel
}

select_arc_cosine_depth <- function(y, base_kernel, max_layers) {
  current_layer <- 1
  current_kernel <- base_kernel

  eigen_current_kernel <- eigen(current_kernel)
  current_rank <- length(which(eigen_current_kernel$values > 1e-10))
  current_vectors <- eigen_current_kernel$vectors[, 1:current_rank, drop = FALSE]
  current_values <- eigen_current_kernel$values[1:current_rank]
  current_projection <- t(current_vectors) %*% scale(y, scale = FALSE)

  current_phi_solution <- optimize(
    function(phi) {
      -0.5 * sum(log(1 + phi * current_values)) -
        (current_rank - 1) / 2 * log(sum(current_projection^2 / (1 + phi * current_values)))
    },
    lower = 0.0005,
    upper = 200,
    maximum = FALSE
  ...
  current_phi <- current_phi_solution$minimum
  current_loglik <- 0.5 * sum(log(1 + current_phi * current_values)) -
    (current_rank - 1) / 2 * log(sum(current_projection^2 / (1 + current_phi * current_values)))

  stop_search <- FALSE

  while (!stop_search && current_layer < max_layers) {
    candidate_layer <- current_layer + 1
    candidate_kernel <- build_arc_cosine_kernel(base_kernel = current_kernel, nl = 1)

    eigen_candidate_kernel <- eigen(candidate_kernel)
    candidate_rank <- length(which(eigen_candidate_kernel$values > 1e-10))
    candidate_vectors <- eigen_candidate_kernel$vectors[, 1:candidate_rank, drop = FALSE]
    candidate_values <- eigen_candidate_kernel$values[1:candidate_rank]
    candidate_projection <- t(candidate_vectors) %*% scale(y, scale = FALSE)

    candidate_phi_solution <- optimize(
      function(phi) {
        -0.5 * sum(log(1 + phi * candidate_values)) -
          (candidate_rank - 1) / 2 * log(sum(candidate_projection^2 / (1 + phi * candidate_values)))
      },
      lower = 0.0005,
      upper = 200,
      maximum = FALSE
    ...
    candidate_phi <- candidate_phi_solution$minimum
    candidate_loglik <- 0.5 * sum(log(1 + candidate_phi * candidate_values)) -
      (candidate_rank - 1) / 2 * log(sum(candidate_projection^2 / (1 + candidate_phi * candidate_values)))

    if (candidate_loglik < current_loglik) {
      stop_search <- TRUE
    } else {
      current_kernel <- candidate_kernel
...
```{r gak}
GAK1 <- build_arc_cosine_base_kernel(Geno)
dim(GAK1)
GAK1[1:5, 1:5]
...
Gl <- select_arc_cosine_depth(y = Pheno$GY, base_kernel = G, max_layers = 50)
...
GAK <- build_arc_cosine_kernel(base_kernel = G.L, nl = Gl)
...
```{r pak}
PAK1 <- build_arc_cosine_base_kernel(NIR.all)
dim(PAK1)
PAK1[1:5, 1:5]
...
Pl <- select_arc_cosine_depth(y = Pheno$GY, base_kernel = P, max_layers = 50)
...
PAK <- build_arc_cosine_kernel(base_kernel = P.L, nl = Pl)
