...
This chunk defines the knitting options for the document so that all code, messages, and figures follow the same presentation standard.
...
This chunk loads the packages required for matrix construction, data wrangling, kernel calculations, optimization, and plotting.
...
This chunk reads the NIR data and prepares the phenomic input that will later be matched to the phenotypic and genomic records.
...
This chunk reads the genomic marker data that provide the basis for the genomic kernels used in the downstream models.
...
This chunk assembles the phenotypic table that serves as the observation-level backbone for the aligned matrices.
...
This chunk harmonizes genotype identifiers across data sources so that genomic, phenomic, and phenotypic information refer to the same materials.
...
This chunk organizes the harmonized data by environment to support later checks of balance, overlap, and kernel construction.
...
This chunk identifies the genotypes that are shared across environments, which is important for stable cross-environment alignment.
...
This chunk constructs the linear genomic kernel used as the basic genomic relationship representation in the pipeline.
...
This chunk constructs the linear phenomic kernel from the NIR information so it can be compared with the genomic and weather-based structures.
...
This chunk builds the categorical environment matrix that represents the macro-environment effect separately from the weather-derived kernel.
...
This chunk reads the engineered climate outputs produced by the climate module so that the weather kernel can be constructed from saved files.
...
This chunk standardizes the climate covariates to place the weather variables on a comparable scale before kernel construction.
...
This chunk derives the year-level weather signatures that summarize the standardized climate information for kernel building.
...
This chunk constructs the weather kernel `W`, which complements rather than replaces the categorical environment structure.
...
This chunk summarizes the basic linear kernels so the reader can verify dimensions, ranges, and general behavior before moving on.
...
This chunk performs direct spot checks on selected entries to confirm that the aligned matrices are behaving as expected.
...
This chunk runs explicit alignment checks among matrices and metadata to catch mismatches before the nonlinear kernels are built.
...
This chunk constructs the linear interaction kernels that link genomic or phenomic information with environment and weather structures.
...
This chunk defines the Gaussian-kernel helper functions that are needed to optimize and generate the nonlinear genomic and phenomic kernels.
...
This chunk computes the candidate genomic Gaussian kernel values before the final bandwidth choice is fixed.
...
This chunk searches for the genomic Gaussian-kernel parameter that best matches the intended kernel behavior.
...
This chunk reads and inspects the selected solution for the genomic Gaussian kernel so the final object can be built explicitly.

```{r ggk-read-solution, include=FALSE}
solG <- readRDS("output/Matrizes/solG.RDS")
```

This chunk constructs the final genomic Gaussian kernel using the chosen parameter value.
...
This chunk computes the candidate phenomic Gaussian kernel values before the final bandwidth is selected.
...
This chunk searches for the phenomic Gaussian-kernel parameter used to define the final phenomic nonlinear kernel.
...
This chunk reads and checks the selected solution for the phenomic Gaussian kernel before final construction.

```{r pgk-read-solution, include=FALSE}
solP <- readRDS("output/Matrizes/solP.RDS")
```

This chunk builds the final phenomic Gaussian kernel with the chosen bandwidth.
...
This chunk generates the Gaussian-kernel interaction structures so the nonlinear kernels can also be combined with environment and weather information.
...
This chunk defines the arc-cosine kernel functions that are needed to compute the additive nonlinear kernels.
...
This chunk constructs the genomic arc-cosine kernel candidate used in the nonlinear genomic pathway.
...
This chunk checks the genomic arc-cosine kernel so the reader can verify that the object is numerically well formed.
...
This chunk constructs the phenomic arc-cosine kernel candidate used in the nonlinear phenomic pathway.
...
This chunk checks the phenomic arc-cosine kernel to confirm that the final matrix is coherent.
...
This chunk builds the interaction kernels associated with the arc-cosine structures so they can be used in the downstream models.
...
This chunk summarizes the nonlinear kernels and their interactions to provide a final diagnostic view of the constructed objects.
...
This chunk draws the diagnostic figure that visually compares the behavior of the main kernels.
...
This chunk saves all matrices and kernels to disk and then performs basic cleanup so that later modules can load the artifacts directly.
