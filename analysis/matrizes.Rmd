...
4. **Weather-derived covariates** (`W`): meteorological descriptors available from the climate preprocessing step.
...
- **Nonlinear arccosine kernels**: `GAK`, `PAK`, `GAKE`, `PAKE`, `GAKW`, `PAKW`
...
- climate-derived covariates available in `output/climate_results/`
...
**Next step**
- the saved kernel objects are used by the variance-component and prediction-analysis stages.
...
In this tutorial, we avoid small convenience shortcuts for plotting and checking matrices. Instead, the code is written in a more explicit and linear way. This makes the script longer, but it also makes each operation easier to follow for readers who are still learning R and matrix-based modeling workflows.

The only routines kept later in the script are the mathematical routines strictly required to estimate the Gaussian and arccosine kernels. Those routines are part of the modeling procedure itself, not programming shortcuts.
...
```{r build-zp}
# Identify spectral variables: keep only numeric columns and remove
...
The weather kernel uses the climate-derived files available in `output/climate_results/`. Two files are required:
...
The annual summaries now include `PTR_AVG`, the average daily photothermal ratio. The expanded daily file also keeps `PHOTOPERIOD_HOURS` as an audit variable and `PTR_DAILY` as a daily weather descriptor. Because `PHOTOPERIOD_HOURS` is an intermediate component used to compute PTR, it is excluded manually from the daily weather kernel, while `PTR_DAILY` remains eligible to enter `ZW`.
...
The code below reads the climate-derived files required to construct the weather kernel.
...
annual_cols <- c(
  "TMAX_AVG", "TMIN_AVG", "GDD_CUM", "PTR_AVG", "HEAT_STRESS_DAYS",
...
# Keep PHOTOPERIOD_HOURS available for auditing, but do not use it
# directly in ZW because PTR_DAILY already combines photoperiod and GDD.
manual_exclude_daily <- intersect(
  c("PHOTOPERIOD_HOURS"),
...
candidate_daily_cols <- setdiff(
  colnames(ECs_expanded),
  c(daily_id_cols, annual_cols, manual_exclude_daily)
)
...
excluded_daily_cols <- c(
  manual_exclude_daily,
  setdiff(candidate_daily_cols, daily_cols)
)
...
### 10.3 Diagnostic figures are prepared after all kernel families are available

We postpone the final diagnostic figures until the Gaussian and arccosine kernels are available. This allows us to compare the linear and nonlinear representations within the genomic and phenomic spaces separately, which is more informative for the article than mixing weather and phenomic matrices in a single panel.
...
The mathematical routines below define the likelihood-based procedure used to tune the nonlinear genomic and phenomic Gaussian kernels.
...
## 13. Build nonlinear arccosine kernels

Arccosine kernels offer a second nonlinear representation. They are inspired by neural-network geometry and can capture hierarchical nonlinear patterns through recursive transformations.

In this tutorial we keep the mathematical functions below explicitly in the script because they define the arccosine kernel itself. Even though they are written as functions, they are part of the method rather than programming shortcuts.

### 13.1 Core mathematical functions for arccosine kernels

The functions below define the arccosine kernels used to compute the additive nonlinear structures.
...
### 13.2 Genomic arccosine kernel (`GAK`)

The arccosine kernels follow the same practical rule adopted for the Gaussian kernels. In the current pipeline, the number of hidden layers is selected **once using GY as the reference trait**, and the resulting kernel is then reused in both trait analyses.

The code below constructs the genomic arccosine kernel candidate used in the nonlinear genomic pathway.
...
The checks below verify that the genomic arccosine kernel is numerically well formed.
...
### 13.3 Phenomic arccosine kernel (`PAK`)

The phenomic arccosine kernel is also calibrated **once using GY as the reference trait**, and the final kernel is reused for both GY and KW in the downstream models.

The code below constructs the phenomic arccosine kernel candidate used in the nonlinear phenomic pathway.
...
The checks below confirm that the phenomic arccosine kernel is coherent.
...
### 13.4 Arccosine interaction kernels

The code below builds the interaction kernels associated with the arccosine structures so they can be used in the downstream models.
...
The same clustering order is then applied to the rows and columns of the matrix. This makes it easier to identify block structure, local similarity patterns, and contrasts among the linear, Gaussian, and arccosine kernels.
...
kernel_plot_info <- data.frame(
  ...
  KernelLabel = c(
    ...
    "Arccosine genomic kernel",
    "Linear phenomic kernel",
    "Gaussian phenomic kernel",
    "Arccosine phenomic kernel"
  ),
  stringsAsFactors = FALSE
)

kernel_plot_objects <- list()
dir.create("output/Matrizes/figures", recursive = TRUE, showWarnings = FALSE)
...
for (i in seq_len(nrow(kernel_plot_info))) {
  ...
  kernel_plot_objects[[current_name]] <- current_plot
  print(current_plot)
}
```

```{r save-kernel-diagnostic-figure-square, include=FALSE}
for (current_name in names(kernel_plot_objects)) {
  ggsave(
    filename = paste0("output/Matrizes/figures/", current_name, "_ordered_heatmap.tiff"),
    plot = kernel_plot_objects[[current_name]],
    width = 8,
    height = 8,
    dpi = 300,
    compression = "lzw",
...
The kernel objects are computationally expensive to reproduce. For that reason, the final step saves them as `.rds` files and cleans the workspace so later modeling stages can start from a lighter memory state.
