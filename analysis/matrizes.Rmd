...
Marker values are standardized **within each environment-specific observation block** before the blocks are stacked, preserving the preprocessing used in the fitted analyses.
...
The nonlinear-kernel tuning documented below uses the complete GY response before cross-validation. The selected Gaussian settings and arccosine recursion depths are then fixed and reused downstream, including for KW. Consequently, nonlinear-kernel tuning is not nested within cross-validation, and the resulting comparisons should be interpreted conditional on this tuning procedure.
