...
This block restores the operational objects used by the current workflow: the fixed leave-one-environment-out set, the five-fold genotype partition, and the `MODEL × REP × TRAIT` task grid for the 18-model catalog. `num_rep = 10` is defined in the preceding setup, so each model-trait combination is evaluated over 10 repetitions, with five fold-level fits or fit targets inside each repetition. The fitting and summarization logic are written directly inside the CV blocks so that the execution path stays explicit.
