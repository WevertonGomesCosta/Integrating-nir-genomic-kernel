...
The code below rebuilds the execution environment when the file is rendered on its own. It respects the contract of `analysis_prediction.Rmd` and recreates the reduced set of 18 ETA objects exactly as defined in Part I.
...
## 12. Note on saved `BGLR` artifacts

This execution stage now stores two different kinds of outputs, and it is useful to distinguish them clearly in tutorial form.

**Auxiliary fitting artifacts saved for diagnostics.**  
During each call to `BGLR(...)`, the argument `saveAt` points to a persistent prefix inside `output/results/bglr_runs/`. These files are generated by `BGLR` while the model is being fitted. They are not the main CV summaries, but they are now preserved because they may be needed later for convergence checks or other diagnostic analyses.

**Final analytical outputs.**  
After the fit finishes, the script collects the predictions, runtime, and CV summaries in memory and then writes the final CSV files to `output/results/`. These CSV files remain the main outputs that will be interpreted in the downstream visualization and summary modules.

**Why both sets of files are useful.**  
The files stored in `output/results/bglr_runs/` document the fitting process itself, whereas the CSV files in `output/results/` summarize the predictive results that are compared across CV schemes, traits, environments, and models. Keeping both layers available makes the workflow more transparent and preserves the materials needed for later convergence analyses.

**Practical interpretation for this module.**  
When this file inventories outputs, it inventories only the final CSV files stored directly under `output/results/`. The `bglr_runs` folder is preserved as an auxiliary archive of fitting artifacts and is not mixed with the final prediction tables.
