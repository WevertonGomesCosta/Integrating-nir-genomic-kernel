...
## 11. Persistent `BGLR` fitting artifacts

In the current 18-model workflow, persistent `saveAt` artifacts are written under `output/results/bglr_runs/` during each fitted model and preserved for later convergence diagnostics. The CV1 fit generates both CV1 holdout predictions and the associated CV2 training-set recovery diagnostic, so their BGLR artifacts are stored under the `CV1` fit path. Likewise, the leave-one-environment-out fit generates both CV0 and CV00 predictions, so its BGLR artifacts are stored under the `CV0` fit path. Final prediction CSV outputs remain separated by the four evaluation labels (`CV1`, `CV2`, `CV0`, and `CV00`). Persisted filenames use the fitted-scenario prefix together with explicit effect labels instead of generic `ETA_1`, `ETA_2`, and similar placeholders.
