...
In the current 18-model workflow, temporary `saveAt` artifacts are written to `tempdir()` during each fit and removed immediately after the runtime information and predictions are collected. Therefore, this execution module inventories only the final CSV outputs stored under `output/results/`.
