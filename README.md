...
Before rerunning the prediction pipeline, remove old files from `output/results/`. The visualization script expects the current filename pattern generated by the updated prediction workflow, especially for `CV1` and `CV2`, which now include both **repetition** and **fold** in the filename.
